...
//global
$type_id = ($_GET['type_id']);
$stmt = mysqli_query($connect_db, "SELECT * FROM app_types WHERE id='$type_id'");
#bind result set
$record = mysqli_fetch_array($stmt);
#fetch location name nad pass as object/variable to field
$oldtype = $record['type_name'];
...
if(isset($_POST['btnUpdate'])){
    ...
    //get form data
    $newtype = ucfirst($_POST['name']);

    //update the record 
    if ($stmt = $connect_db->prepare('UPDATE `app_types` SET `type_name`= ? WHERE id= ?')){
    $stmt->bind_param('si', $newtype, $type_id);
    $stmt->execute();
...
-->
        ...
        <div id="content">
            ...
            <div class="inner" style="min-height: 700px;">
                <div class="row">
                    <div class="col-lg-12">
                       <!--page title/section-->
                        <h5><span class="fa fa-home"></span> E-Permit <i class="fa fa-chevron-right"></i> Main Menu <i class="fa fa-chevron-right"></i> Application Categorys <i class="fa fa-chevron-right"></i> Update </h5>
                    </div>
                </div>
                  <hr />
                ...
                <div class="col-lg-12">
                   <a href="application-types.php" class="btn btn-warning text-center"> <i class="fa fa-arrow-left" style="margin-right: 3px;"></i> Go Back</a>
                    <div class="box">
                        ...
                        <h5>Edit Application Category</h5></header>
        ...
        #bind result set
        $record = mysqli_fetch_array($stmt); ?>
                                <input name="name" id="name" class="form-control" value="<?php echo $record['type_name']; ?>" placeholder=""/>
<!--
...
<!--                                <span><label for="oldlocation"> Name: <?php //echo $oldlocation; ?></label></span>-->
                            ...
                            <div class="col-lg-4">
                            <!--save button-->
                            <button class="btn btn-success" type="submit" name="btnUpdate"><i class="fa fa-paper-plane">
                            </i> Update</button>
                            </div>
                        </div>
