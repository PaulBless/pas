...
if(isset($_POST['btnSubmit'])){
    ...
    ## get form data
    $checkname = ucwords($_POST['checkname']);
    $comment = ucfirst($_POST['comment']);

    //preparing the SQL statement will prevent SQL injection.
    if ($stmt = $connect_db->prepare('SELECT * FROM check_list WHERE check_name= ?')) {
    ...
    echo "<div class='alert alert-danger bounceUp col-lg-6 col-md-offset-3' style='top: 10px; transition: transform .3s ease, -webkit-transform .3s ease, -moz-transform .3s ease, -o-transform .3s ease'>"
        ...
        . "<strong>&nbsp;&nbsp;The record '$checkname' is already added!</strong>."
        . "</div>";
        
    }else{
    //save new record 
   if ($stmt = $connect_db->prepare('INSERT INTO `check_list` (check_name, comments) VALUES (?, ?)')){
	$stmt->bind_param('ss', $checkname, $comment);
	$stmt->execute();
        ...
        <div id="content">
            ...
            <div class="inner" style="min-height: 700px;">
                <div class="row">
                    <div class="col-lg-12">
                       <!--page title/section-->
                        <h5><span class="fa fa-home"></span> E-Permit <i class="fa fa-chevron-right"></i> Main Menu <i class="fa fa-chevron-right"></i> Checklist <i class="fa fa-chevron-right"></i> Add New </h5>
                    </div>
                </div>
                  <hr />
            ...
            <div class="row">
                <div class="col-lg-12">
                   <a href="check-lists.php" class="btn btn-warning text-center"> <i class="fa fa-arrow-left" style="margin-right: 3px;"></i> Go Back</a>
                    <div class="box">
                        <header>
                        <div class="icons"><i class="fa fa-tag"></i>
                        </div>
                        <h5>Add Checklist Item</h5></header>
                        ...
                        <div class="form-group">
                            <label class="control-label col-lg-4">Checklist Item Name  </label>
                            <div class="col-lg-4">
                                <input name="checkname" id="checkname" class="form-control" placeholder="" />
<!--                                <p class="help-block">Provide name of checklist item </p>-->
                            </div>
                        </div>
                        <!--description-->
                        <div class="form-group">
                            ...
                            <div class="col-lg-4">
                                ...
                                <p class="help-block">Provide description of checklist item </p>
                            </div>
                        </div> 
                    
                        <!--button for action-->
                        <div class="form-actions no-margin-bottom" style="text-align:center; padding-top: 20px; padding-left: 250px;">
                            <button class="save_btn btn btn-success " type="submit" name="btnSubmit" style="font-weight: bold"><i class="fa fa-save"></i> Save Record</button>
                        </div>
...
//            valid: 'glyphicon glyphicon-ok',
        ...
        fields: {
            checkname: {
                ...
                validators: {
                    notEmpty: {
                            message: 'Enter checklist item name  '
                        },
                    regexp: {
                        regexp: /^[a-zA-Z ]+$/,
    ...
    //checklist name keypress validation
